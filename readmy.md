...
***git commit -m "comments"*** - команда сохраняет текущее состояние файла;

***git commit -a -m "comments"*** - команда объединяющая **"git add"** и **"git commit"**;

***git status*** - команда проверяет статус репозитория, все ли сохранено;

***git log*** - команда выводящая на экран  журнал изменений, с указанием индекса каждого комита;

***git checkout*** - команда позволяющая осуществлять переход к более ранней версии файла и обратно.